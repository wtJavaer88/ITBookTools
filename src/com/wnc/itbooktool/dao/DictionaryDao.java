...
import com.wnc.itbooktool.word.WordExpand;
...
public class DictionaryDao {
	...
	public synchronized static DicWord findWordById(int dict_id) {
		...
		try {
			String sql = "select e.word_done,e.word_er,e.word_est,e.word_ing,e.word_pl,e.word_past,e.word_third,d.topic_id,d.topic_word,d.mean_cn FROM  dictionary D LEFT JOIN word_exchange E  ON E.dict_id=D.id where D.id="
					+ dict_id;
	...
	public synchronized static WordExpand findSameAntonym(int topic_id) {
		...
		try {
			...
			for (int i = 1; i <= selectAllSqlMap.size(); i++) {
				...
				wordExpand.setTopic_id(getMapInt(rowMap, "topic_id"));
				wordExpand.setAntonym(getMapStr(rowMap, "antonym_words"));
				wordExpand.setSame(getMapStr(rowMap, "similar_words"));
				wordExpand.setSame_analysis(getMapStr(rowMap, "same_analysis"));
				return wordExpand;
			}
		} catch (Exception e) {
		}
