...
public class CBListener extends Thread {
	...
	private String getAlertText(String clipBoardContent, DicWord findWord) {
		return String.format("cmd /c %s \"%s\" \"%s\"", MyAppParams.getAlertExe(),
				findWord.getCn_mean().replaceAll("['\"]", ""), clipBoardContent);
	}
	 ...
	 *            1为找到单词, 2为没找到单词, 3为句子
	 */
	private void writeToBookLog(String clipBoardContent, DicWord object, int type) {
		...
		BasicFileUtil.writeFileString(MyAppParams.getInstance().getTodayLogPath(), jobj.toJSONString() + "\r\n", null,
				true);
	}
}
