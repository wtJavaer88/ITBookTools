...
public class SeekITWords
{
    public static void main( String[] args )
    {
        ...
        for ( String string : readFrom )
        {
            ...
            if ( !DbExecMgr
                    .isExistData( "SELECT * FROM DICTIONARY WHERE TOPIC_WORD='"
                            + string.split( " " )[0] + "'" ) )
                seek( string );
        }
    }
    ...
    private static void doDict( String word, String weight,
            BaiduWordTranslate baiduWordTranslate )
    {
        try
        {
            ...
            JSONObject jsonObject0 = baiduWordTranslate.getJsonObject()
                    .getJSONObject( "dict_result" )
            ...
            for ( int i = 0; i < jsonArray.size(); i++ )
            {
                ...
                if ( i != 0 )
                {
                    means += "\n" + jsonObject.getString( "part" )
                            + jsonObject.getString( "means" );
                } else
                {
                    means += jsonObject.getString( "part" )
                            + jsonObject.getString( "means" );
                }
            }
            ...
            String sql = "INSERT INTO Dictionary(TOPIC_WORD, MEAN_CN, ACCENT, ORGINAL, WEIGHT) VALUES('"
                    ...
                    + "','"
                    + StringEscapeUtils.escapeSql( means )
                    + "','"
                    + StringEscapeUtils.escapeSql( am )
                    + "','"
                    + StringEscapeUtils.escapeSql( orginal )
                    + "',"
                    + weight
                    + ")";
    ...
    private static void doWE( String word, BaiduWordTranslate baiduWordTranslate )
    {
        try
        {
            ...
            if ( wordExchange != null
                    && (isNotNull( wordExchange.getWord_done() )
                            || isNotNull( wordExchange.getWord_done() )
                            || isNotNull( wordExchange.getWord_er() )
                            || isNotNull( wordExchange.getWord_est() )
                            || isNotNull( wordExchange.getWord_ing() )
                            || isNotNull( wordExchange.getWord_past() )
                            || isNotNull( wordExchange.getWord_pl() ) || isNotNull( wordExchange
                                .getWord_third() )) )
            {
    ...
    private static String getOrigin( BaiduWordTranslate baiduWordTranslate )
            throws Exception
    {
        String string = "";
        try
        {
            string = baiduWordTranslate.getJsonObject()
                    .getJSONObject( "dict_result" ).getJSONObject( "collins" )
                    .getString( "word_name" );
        } catch ( Exception e )
        {
        }
